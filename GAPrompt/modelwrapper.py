...
def convert_to_t5_inputs(dataset,tokenizer,max_length):
    ...
    # print(torch.tensor(inputs['decoder_input_ids']).shape)
    return inputs
...
class FitnessModelWrapper:
    ...
    def __batch_eval(self,unlabeled_data,labels,label_ids):
        ...
        if self.__is_t5():
            mask_index = torch.where(input["decoder_input_ids"] == self.mask_token_id)
            # input["decoder_input_ids"] = torch.squeeze(input["decoder_input_ids"])
        elif self.__is_gpt2():
    ...
    def fitness(self,individuals,data, tpu = False, test=False):
        ...
        # if tpu: 
        #     results, preds, fits, label_log_probs = xmp.spawn(self.__tpu_eval,args=(eval_dataset,labels,label_ids),start_method='fork')
        #     # results, preds, fits, label_log_probs = self.__tpu_eval(0,eval_dataset,labels,label_ids)
        # else:
        ...
        for i in range(len(individuals)):
            ...
            if test:
                print(individuals[i].prompts, individuals[i].fitness_score, individuals[i].accuracy)
