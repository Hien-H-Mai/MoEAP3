    ...
    def __init__(self, queue, device_id, model, data_sample, args):
        ...
        if global_bias == None:
            ...
            if isinstance(self.model.model,T5ForConditionalGeneration):
                ...
                global_bias = torch.zeros(len(self.model.tokenizer))
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                # print("## mask token", self.model.tokenizer.mask_token_id)
...
class FitWorker(Thread):
    def __init__(self, queue, device_id, model, train_data, test = False):
        Thread.__init__(self)
        self.queue = queue
        self.device_id = device_id
        self.model = copy.copy(model)
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                # print("Fit")
                label_log_probs = self.model.fitness([individual],self.train_data, self.test)
